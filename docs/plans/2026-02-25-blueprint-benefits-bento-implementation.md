# Blueprint Benefits Bento Grid Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the sticky stacking cards in the "Why Start With a Blueprint?" section with a modern 2x2 Bento Grid layout, and apply a scrolly-tell animation to the introductory text.

**Architecture:** We will rewrite `BenefitStackSection.tsx`. We'll remove the artificial 400vh scroll height and sticky behavior. We'll introduce a `Word` component for the scrolly-tell text reveal (similar to `ScrollytellSection`), and a CSS Grid (`grid-cols-1 md:grid-cols-2`) for the Bento layout. Each benefit card will be a glassmorphic container that gently fades and scales up on scroll.

**Tech Stack:** React, Tailwind CSS, Framer Motion, Lucide Icons

---

### Task 1: Rewrite BenefitStackSection.tsx

**Files:**
- Modify: `src/components/marketing/BenefitStackSection.tsx`

**Step 1: Write the updated implementation**

We will completely replace the contents of `BenefitStackSection.tsx` with the new design.
...
1. The introductory text highlights word-by-word as you scroll down.
2. The 2x2 grid appears smoothly below it.
3. The cards exhibit a smooth glassmorphic hover effect and icon scaling.

**Step 3: Commit**
```bash
git add src/components/marketing/BenefitStackSection.tsx
git commit -m "feat: implement bento grid and scrolly-tell for blueprint benefits"
```
